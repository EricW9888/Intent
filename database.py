...
# The GUI writes from several daemon threads at once, so give SQLite room to wait
# out a lock instead of raising "database is locked" on the first collision.
...
@contextmanager
def _session(db_path: Path = DB_PATH) -> Iterator[sqlite3.Connection]:
